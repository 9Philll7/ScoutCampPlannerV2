...
- Local Docker operation stores a randomly generated 32-byte key in a dedicated persistent secret volume outside PostgreSQL. The ASP.NET Core container runs as the dedicated non-root `app` identity; the volume is mounted only into that container and files are readable only by that identity. The key is never baked into an image, committed to Git, or passed through a normal environment variable. Backup and recovery procedures treat the key file and protected checkpoint as required companions to the database backup.
...
Audit segment boundaries are independent of key rotation. The active segment is closed at the first safe append opportunity after a UTC calendar-month change, and a new segment is started with the same active key. Both system events are committed together, retain the monotonically increasing instance sequence, and link the new starting event directly to the old closing HMAC. An explicit key rotation creates an additional immediate segment boundary using its separate two-key transition. No empty segment is required for a month without audit events.
...
Database, complete audit key bundle, and authenticated checkpoint form one recovery set. A single-device recovery archive decrypts the DPAPI-protected key only inside the application process, packages all three components, and encrypts the complete archive with a user-supplied backup password using Argon2id and AES-256-GCM. This permits recovery under a different Windows user or on a replacement device; the restored key bundle is protected again with the destination user's DPAPI context. The backup password must satisfy the productive password policy and is never stored in the archive. Cloud and local Docker deployments may use provider-native backup tooling, but their database snapshot, deployment secret or security volume, and checkpoint must be captured and restored as the same logical recovery set.

Backup creation blocks writes, reconciles the checkpoint, and verifies the full audit chain before capturing the set. Restore occurs into staging while normal operation remains blocked. Database integrity, key bundle, checkpoint, and the full chain must all verify before the restored set replaces active state. The operational sequence is defined in [`audit-backup-and-restore.md`](../architecture/audit-backup-and-restore.md).
...
The focused validation is recorded in [`audit-security-validation.md`](../spike/audit-security-validation.md). Phase 1 confirms a dependency-free canonical UTF-8 JSON candidate, identical golden bytes on Windows and Linux, HMAC-chain verification, manipulation detection, and initial in-memory performance. Phase 2 confirms atomic business/event/database-head transactions, idempotent recovery after interrupted external-checkpoint advancement, and safe concurrent append allocation using a SQLite process gate and PostgreSQL head-row locking. Phase 3 confirms prepared-key staging, bidirectionally signed segment boundaries, historic verification, rotation recovery, versioned key-bundle and authenticated-checkpoint files, atomic file replacement, Windows CurrentUser DPAPI, and a non-root Docker security volume isolated from PostgreSQL. Provider round trips preserve canonical bytes across a persisted multi-segment transition. Provider-backed load-and-verify measurements for 100,000 events support the bounded 1,000-event startup policy. A framework-free policy spike confirms fail-closed operation, narrowly scoped read-only continuity, local-only recovery, and release only after successful full verification. Productive Platform contracts and adapters now load Windows DPAPI files, restrictive Docker-volume files, or a deployment-provided cloud secret without silently replacing missing existing material. The encrypted three-part recovery-set probe confirms cross-device restoration and tamper rejection. Monthly same-key segment transitions, conservative whole-segment retention, and transfer-scoped package-version-2 binding are also validated. The focused ADR-012 security spike is accepted; productive audit and package-v2 implementation remain separate work.
...
- The deletion eligibility date of a segment is the latest retention-expiry date of any event it contains. A shorter-lived event does not permit partial deletion or rewriting of that segment.
- A legal hold or an active offline transfer affecting any event blocks deletion of the complete segment.
- Monthly segment boundaries provide bounded deletion granularity without forcing calendar-based key rotation.
...
At offline preparation, the cloud creates one ECDSA P-256 key pair scoped to the transfer. It stores the public key with the transfer ID, tenant ID, camp ID, and baseline version. The private key is carried only inside the password-protected cloud-to-local package and is removed from the local instance when the transfer is administratively completed. It never becomes an application-instance audit key.

Both package directions use an Argon2id-derived AES-256-GCM envelope with separate direction-specific purpose values. The one-time transfer password satisfies the productive password policy and is communicated separately from the package file. The local-to-cloud signature covers the versioned transfer context, source instance ID, inclusive audit sequence range, first predecessor, final head, hash of the complete domain payload, and hash of every length-delimited canonical audit event plus its HMAC. Consequently, neither domain data nor the audit section can be moved to another transfer, tenant, camp, or baseline.

Local audit HMAC keys are never included in a package. Before signing, the local instance fully verifies the exported section against its protected checkpoint and keys. The cloud verifies sequence continuity, predecessor links, the signed section digest, and the signature against the public key recorded when the transfer began. This preserves instance-local HMAC key ownership while making substitution or truncation of transferred evidence detectable.
...
The version-2 security candidate validates package encryption, direction separation, transfer-scoped key provisioning, return signatures, context and payload binding, mandatory audit continuity, and duplicate classification. Productive package-version migration, canonical container serialization, compatibility fixtures, persistent transfer-key records, and transactional audit ingestion remain implementation work. This ADR does not weaken the version 1 rule that Platform user data is not replaced.
...
- The accepted integrity and offline-transfer models have completed their focused technical validation. Productive implementation and package-version-2 compatibility work remain required before release. Retention requires organisational/legal confirmation before production use.
