...
Tenant authentication and the first basic Camp workflow are now implemented: an authorized tenant user can create a named and dated camp, explicitly assign at least one active tenant member as `CampAdmin`, see only camps granted through an active camp membership, and change name or period with `camp.edit`. The Camp structure is defined as a neutral tree with freely named levels; its first increment supports listing and creating nodes in free mode. The next Camp increments should add fixed structure templates plus constrained move and delete operations before participants are assigned to leaf nodes. Cooking-unit assignment belongs to Catering and is not part of the Camp structure. Catering, Finance, Program, Material, and expanded participant/health functionality should follow as separate increments.
