...
- cross-module business/audit transaction coordination, checkpoint advancement, verification, and use-case integration; the Platform-owned contract, EF migrations, active-key loading, canonical HMAC encoding, journal initialization, standalone append transaction, and Platform-local shared business/audit transaction are present for PostgreSQL and SQLite
