...
- final Argon2id parameter calibration, versioned password denylist, Unicode password-length counting, audit/package-security spike validation, and legal retention review; authentication, identity, authorization, the initial password-security libraries, and the audit transfer direction are defined by ADR-009 through ADR-012 and the focused security-library validation
