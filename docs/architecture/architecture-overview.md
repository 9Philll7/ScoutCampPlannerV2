...
- strength-check library selection, final Argon2id parameter calibration, audit/package-security spike validation, and legal retention review; authentication, identity, authorization, the initial Argon2id library, and the audit transfer direction are defined by ADR-009 through ADR-012 and the focused security-library validation
