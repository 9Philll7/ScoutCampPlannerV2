...
public sealed class DatabaseMigrationTests
{
    ...
    public async Task Sqlite_upgrade_preserves_existing_data_and_applies_each_module_history()
    {
        ...
        Assert.Equal(9, await ScalarAsync<long>(connection, "SELECT COUNT(*) FROM __EFMigrationsHistory_catering"));
        ...
        Assert.Equal(17, await ScalarAsync<long>(connection, "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name IN ('Recipes', 'RecipeDraftTags', 'RecipeGroups', 'RecipeIngredientPositions', 'RecipeIngredientReplacements', 'RecipeIngredientReplacementAllergens', 'RecipeIngredientReplacementIntolerances', 'RecipeIngredientReplacementDietaryRequirements', 'RecipeSubrecipePositions', 'RecipeSubrecipeReplacements', 'RecipeSubrecipeReplacementAllergens', 'RecipeSubrecipeReplacementIntolerances', 'RecipeSubrecipeReplacementDietaryRequirements', 'RecipeRevisions', 'RecipeRevisionWarnings', 'TenantRecipeEntries', 'CampRecipeEntries')"));
    ...
    public async Task PostgreSql_upgrade_preserves_existing_data_and_applies_each_module_history()
    {
        ...
        Assert.Equal(9, await ScalarAsync<long>(connection, "SELECT COUNT(*) FROM catering.\"__EFMigrationsHistory\""));
