...
public sealed class DatabaseMigrationTests
{
    ...
    public async Task Sqlite_upgrade_preserves_existing_data_and_applies_each_module_history()
    {
        ...
        Assert.Equal(17, await ScalarAsync<long>(connection, "SELECT COUNT(*) FROM __EFMigrationsHistory_catering"));
    ...
    public async Task PostgreSql_upgrade_preserves_existing_data_and_applies_each_module_history()
    {
        ...
        Assert.Equal(17, await ScalarAsync<long>(connection, "SELECT COUNT(*) FROM catering.\"__EFMigrationsHistory\""));
        Assert.Equal(9, await ScalarAsync<long>(connection,
        ...
        Assert.Equal(1, await ScalarAsync<long>(connection,
            "SELECT COUNT(*) FROM information_schema.constraint_column_usage WHERE table_schema = 'catering' AND table_name = 'IngredientRevisions' AND column_name = 'Id' AND constraint_name = 'FK_RecipeIngredientPositions_IngredientRevisions_IngredientRevisionId'"));
